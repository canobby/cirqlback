  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#160a18", color: "#ffe6f5", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[520px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff8ab5", textShadow: "0 0 10px rgba(255,138,181,.5)" }}>Sugar Swap</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="sugarswap-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,138,181,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      {/* controls: d-pad moves the cursor · JUMP selects/swaps (or just tap two candies) */}
      <div className="flex w-full max-w-[520px] items-end justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        ...
        <button {...hold("a")} data-testid="btn-jump" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[13px] font-extrabold active:scale-90" style={{ borderColor: "#ff8ab5", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,138,181,.3), rgba(22,10,24,.9))", boxShadow: "0 0 26px rgba(255,138,181,.4)", touchAction: "none" }}>SWAP</button>
      </div>
    </div>
  );
}
