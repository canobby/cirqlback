...
// Puzzle Bobble on a circle: shoot coloured bubbles OUTWARD from the centre into
// rotating rings, match 3+ to pop, keep the creeping field off the centre. A thin
// React host around `PopEngine`; guests play, logged-in players persist best +
// submit the Daily board under gameId 'pop'.
...
function AimWheel({ onAim }: { onAim: (a: number) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute h-7 w-7 rounded-full" style={{ left: SZ / 2 + Math.cos(ang) * R, top: SZ / 2 + Math.sin(ang) * R, transform: "translate(-50%,-50%)", background: "radial-gradient(circle at 38% 34%, #fff, #a78bfa 75%)", boxShadow: "0 0 15px rgba(167,139,250,.8)" }} />
...
const Dot = ({ color, size = 18 }: { color: string; size?: number }) => (
  <span className="inline-block rounded-full" style={{ width: size, height: size, background: `radial-gradient(circle at 35% 32%, #fff, ${color} 72%)`, boxShadow: `0 0 8px ${color}` }} />
);

export default function Pop() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<PopEngine | null>(null);
  const userRef = useRef(user); userRef.current = user;

  const [phase, setPhase] = useState<"menu" | "playing" | "over">("menu");
  const [hud, setHud] = useState<PopHud | null>(null);
  const [result, setResult] = useState<PopResult | null>(null);
  const [best, setBest] = useState(() => +(lsGet("cpop_best") || 0));
  const [sound, setSound] = useState(() => lsGet("cpop_sound") !== "0");
  const [haptics, setHaptics] = useState(() => lsGet("cpop_hap") !== "0");
  const [dailyRank, setDailyRank] = useState<{ rank: number; total: number } | null>(null);

  useEffect(() => {
    if (!canvasRef.current) return;
    const eng = new PopEngine(canvasRef.current, {
      sound, haptics,
      onHud: setHud,
      onRunEnd: (r) => {
        setResult(r); setPhase("over"); setBest(r.best);
        if (r.best > +(lsGet("cpop_best") || 0)) lsSet("cpop_best", String(r.best));
        const u = userRef.current;
        if (!u) return;
        fetch("/api/game/progress", {
          method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ gameId: "pop", state: { best: r.best, settings: { sound, haptics } } }),
        }).catch(() => {});
        setDailyRank(null);
        fetch("/api/game/daily/score", {
          method: "POST", credentials: "include", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ gameId: "pop", score: r.score, bestCombo: r.comboMax, restored: false }),
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    fetch("/api/game/progress?gameId=pop", { credentials: "include" })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (cancelled || !d) return;
        const st = d.state || {};
        if (typeof st.best === "number" && st.best > best) { setBest(st.best); lsSet("cpop_best", String(st.best)); }
        const s = st.settings;
  ...
  const toggleSound = () => { const v = !sound; setSound(v); lsSet("cpop_sound", v ? "1" : "0"); engineRef.current?.setMuted(!v); };
  const toggleHap = () => { const v = !haptics; setHaptics(v); lsSet("cpop_hap", v ? "1" : "0"); engineRef.current?.setHaptics(v); };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "radial-gradient(120% 90% at 50% 8%, #0d0a24 0%, #05040f 60%, #030208 100%)", color: "#e6e9ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold tracking-wide" style={{ background: "linear-gradient(90deg,#f472b6,#a78bfa,#38bdf8)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>
          Cirql Pop
        </div>
        {phase === "playing" && (
          <>
            {hud && hud.combo >= 2 && <div className="ml-auto text-sm font-extrabold text-amber-300">×{hud.combo}</div>}
            <div className={`${hud && hud.combo >= 2 ? "" : "ml-auto"} text-right leading-none`}>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center">
        ...
        {phase === "menu" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center" style={{ background: "radial-gradient(60% 50% at 50% 45%, rgba(10,8,30,.82), rgba(5,4,15,.94))" }}>
            <div className="text-[10px] uppercase tracking-[0.3em] text-violet-300/50">Bubble shooter · reimagined</div>
            <h1 className="text-3xl font-extrabold" style={{ textWrap: "balance", background: "linear-gradient(90deg,#f472b6,#a78bfa,#38bdf8)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>Pop from the centre out</h1>
        ...
        {phase === "over" && result && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center" style={{ background: "radial-gradient(60% 50% at 50% 45%, rgba(10,8,30,.86), rgba(5,4,15,.96))" }}>
            <div className="text-[10px] uppercase tracking-[0.3em] text-rose-300/60">The rings closed in</div>
            <h1 className="text-3xl font-extrabold text-white">Buried</h1>
            ...
            <div className="flex gap-3">
              <button onClick={startRun} data-testid="button-pop-again" className="rounded-full px-8 py-3 text-[15px] font-extrabold tracking-wide active:scale-95" style={{ color: "#180a1e", background: "linear-gradient(90deg,#f472b6,#a78bfa)", boxShadow: "0 8px 30px rgba(167,139,250,.5)" }}>Play again</button>
              <button onClick={toMenu} data-testid="button-pop-menu" className="rounded-full border border-violet-400/30 px-6 py-3 text-[15px] font-extrabold tracking-wide text-violet-200 active:scale-95">Menu</button>
            </div>
          </div>
        )}
      </div>
      ...
      {phase === "playing" && (
        <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-2">
          <button
            onClick={() => engineRef.current?.swap()}
            data-testid="button-swap"
            className="flex h-[74px] w-[74px] flex-col items-center justify-center gap-1 rounded-full border-[1.5px] text-[10px] font-extrabold tracking-wide transition active:scale-90"
            style={{ borderColor: "#a78bfa", color: "#c4b5fd", background: "rgba(255,255,255,.03)" }}
          >
            {hud && <Dot color={hud.next} size={16} />}
            SWAP
          </button>
          ...
          <button
            ...
            className="flex h-[74px] w-[74px] flex-col items-center justify-center gap-1 rounded-full border-[1.5px] text-[10px] font-extrabold tracking-wide transition active:scale-90"
            style={{ borderColor: hud?.current || "#f472b6", color: "#fff", background: "rgba(255,255,255,.03)" }}
          >
            {hud && <Dot color={hud.current} size={22} />}
            SHOOT
          </button>
        </div>
      )}
