...
export default function PiggyBank() {
  ...
  const padBtn = "flex h-12 w-12 items-center justify-center rounded-xl border-[1.5px] active:scale-90";
  const padStyle = { borderColor: "#3bb6ff", background: "rgba(255,255,255,.03)", boxShadow: "0 0 14px rgba(59,182,255,.2) inset", touchAction: "none" as const };

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0e1424", color: "#eaf2ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[520px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffd24a", textShadow: "0 0 10px rgba(255,210,74,.5)" }}>Piggy Bank</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="piggybank-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(59,182,255,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      {/* controls: d-pad slides the whole board · JUMP to (re)start */}
      <div className="flex w-full max-w-[520px] items-end justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 grid-rows-3 gap-1.5" style={{ width: 156 }}>
          <div /><button {...hold("up")} data-testid="btn-up" className={padBtn} style={padStyle}><ChevronUp className="h-6 w-6 text-cyan-200" /></button><div />
          <button {...hold("left")} data-testid="btn-left" className={padBtn} style={padStyle}><ChevronLeft className="h-6 w-6 text-cyan-200" /></button><div /><button {...hold("right")} data-testid="btn-right" className={padBtn} style={padStyle}><ChevronRight className="h-6 w-6 text-cyan-200" /></button>
          <div /><button {...hold("down")} data-testid="btn-down" className={padBtn} style={padStyle}><ChevronDown className="h-6 w-6 text-cyan-200" /></button><div />
        </div>
        <button {...hold("a")} data-testid="btn-jump" className="flex h-20 w-20 flex-col items-center justify-center gap-0.5 rounded-full border-[2px] text-[11px] font-extrabold active:scale-90" style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,210,74,.3), rgba(14,20,36,.9))", boxShadow: "0 0 24px rgba(255,210,74,.4)", touchAction: "none" }}>NEW</button>
      </div>
    </div>
  );
}
