...
import { DozenEngine } from "@/game/dozen-engine";
...
export default function Dozen() {
  ...
  const engineRef = useRef<DozenEngine | null>(null);
  ...
  const dstyle = { borderColor: "#ff9ec2", background: "rgba(255,255,255,.03)", boxShadow: "0 0 16px rgba(255,158,194,.22) inset", touchAction: "none" as const };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0b0712", color: "#fff4ea", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffcf4a", textShadow: "0 0 10px rgba(255,207,74,.5)" }}>Dozen</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="dozen-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,207,74,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 grid-rows-3 gap-1.5" style={{ width: 176 }}>
          <div />
          <button {...hold("up")} data-testid="btn-up" className={dcls} style={dstyle}><ChevronUp className="h-6 w-6" style={{ color: "#ff9ec2" }} /></button>
          <div />
          <button {...hold("left")} data-testid="btn-left" className={dcls} style={dstyle}><ChevronLeft className="h-6 w-6" style={{ color: "#ff9ec2" }} /></button>
          <div className="flex items-center justify-center"><div className="h-3 w-3 rounded-full" style={{ background: "#ff9ec255" }} /></div>
          <button {...hold("right")} data-testid="btn-right" className={dcls} style={dstyle}><ChevronRight className="h-6 w-6" style={{ color: "#ff9ec2" }} /></button>
          <div />
          <button {...hold("down")} data-testid="btn-down" className={dcls} style={dstyle}><ChevronDown className="h-6 w-6" style={{ color: "#ff9ec2" }} /></button>
          <div />
        </div>
      </div>
    </div>
