...
import { ArrowLeft, ChevronUp, ChevronDown, Coffee } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { CuppaRushEngine } from "@/game/cuppa-rush-engine";
...
export default function CuppaRush() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<CuppaRushEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0b0712", color: "#fff4ea", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffb020", textShadow: "0 0 10px rgba(255,176,32,.5)" }}>Cuppa Rush</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="cuppa-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,163,0,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      {/* two-thumb controls: move (left) · serve (right) */}
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="flex flex-col gap-2">
          <button {...hold("up")} data-testid="btn-up" className="flex h-16 w-16 items-center justify-center rounded-2xl border-[1.5px] active:scale-90" style={{ borderColor: "#ffb020", background: "rgba(255,255,255,.03)", boxShadow: "0 0 18px rgba(255,176,32,.25) inset", touchAction: "none" }}><ChevronUp className="h-7 w-7 text-amber-300" /></button>
          <button {...hold("down")} data-testid="btn-down" className="flex h-16 w-16 items-center justify-center rounded-2xl border-[1.5px] active:scale-90" style={{ borderColor: "#ffb020", background: "rgba(255,255,255,.03)", boxShadow: "0 0 18px rgba(255,176,32,.25) inset", touchAction: "none" }}><ChevronDown className="h-7 w-7 text-amber-300" /></button>
        </div>
        <button {...hold("a")} data-testid="btn-serve" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#ff8a3d", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,138,61,.28), rgba(20,12,6,.9))", boxShadow: "0 0 26px rgba(255,138,61,.4)", touchAction: "none" }}>
          <Coffee className="h-7 w-7" style={{ color: "#ffce9a" }} /> SERVE
        </button>
      </div>
    </div>
  );
