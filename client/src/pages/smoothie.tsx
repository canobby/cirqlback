...
export default function Smoothie() {
  ...
  const padStyle = { borderColor: "#ff8ab5", background: "rgba(255,255,255,.03)", boxShadow: "0 0 14px rgba(255,138,181,.2) inset", touchAction: "none" as const };

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#160c18", color: "#ffe6f5", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[520px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff8ab5", textShadow: "0 0 10px rgba(255,138,181,.5)" }}>Smoothie</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="smoothie-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,138,181,.18)", borderRadius: 6, touchAction: "none" }} />
        {over && daily && (
      ...
      {/* move the pour spout · DROP (or just touch the cup) */}
      <div className="flex w-full max-w-[520px] items-center justify-between gap-4 px-8 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="flex gap-2.5">
          <button {...hold("left")} data-testid="btn-left" className={padBtn} style={padStyle}><ChevronLeft className="h-7 w-7 text-pink-200" /></button>
          <button {...hold("right")} data-testid="btn-right" className={padBtn} style={padStyle}><ChevronRight className="h-7 w-7 text-pink-200" /></button>
        </div>
        <button {...hold("a")} data-testid="btn-drop" className="flex h-20 w-20 flex-col items-center justify-center rounded-full border-[2px] text-[13px] font-extrabold active:scale-90" style={{ borderColor: "#ff8ab5", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,138,181,.3), rgba(22,12,24,.9))", boxShadow: "0 0 24px rgba(255,138,181,.4)", touchAction: "none" }}>DROP</button>
      </div>
    </div>
  );
}
