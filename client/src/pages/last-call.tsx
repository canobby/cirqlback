...
export default function LastCall() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#120a06", color: "#ffe6c2", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffd24a", textShadow: "0 0 10px rgba(255,210,74,.5)" }}>Last Call</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="lastcall-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,210,74,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-center px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <button {...hold("a")} data-testid="btn-roll" className="flex h-24 w-52 items-center justify-center gap-2 rounded-full border-[2px] text-[16px] font-extrabold active:scale-95" style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,210,74,.28), rgba(20,14,4,.9))", boxShadow: "0 0 26px rgba(255,210,74,.4)", touchAction: "none" }}>
          <Beer className="h-7 w-7" style={{ color: "#ffe0a0" }} /> ROLL
        </button>
      </div>
    </div>
  );
