...
import { PlinkEngine } from "@/game/plink-engine";
...
export default function Plink() {
  ...
  const engineRef = useRef<PlinkEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0a1020", color: "#dfe8ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#7be0ff", textShadow: "0 0 10px rgba(123,224,255,.5)" }}>Plink</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="plink-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(123,224,255,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-center px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <button {...hold("a")} data-testid="btn-drop" className="flex h-24 w-52 items-center justify-center gap-2 rounded-full border-[2px] text-[16px] font-extrabold active:scale-95" style={{ borderColor: "#7be0ff", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(123,224,255,.28), rgba(6,14,22,.9))", boxShadow: "0 0 26px rgba(123,224,255,.4)", touchAction: "none" }}>
          <ChevronsDown className="h-7 w-7" style={{ color: "#bfeeff" }} /> DROP
        </button>
      </div>
    </div>
  );
