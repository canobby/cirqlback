...
import { ArrowLeft, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Bone } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { BarkParkEngine } from "@/game/bark-park-engine";
...
export default function BarkPark() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<BarkParkEngine | null>(null);
  ...
  const pc = { borderColor: "#33e650", background: "rgba(255,255,255,.03)", touchAction: "none" as const, height: 52, width: 52 };

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0a1a0a", color: "#dff2ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#33e650", textShadow: "0 0 10px rgba(51,230,80,.5)" }}>Bark Park</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="bark-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(51,230,80,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 gap-1" style={{ gridTemplateAreas: "'. up .' 'left down right'" }}>
          <button {...hold("up")} data-testid="btn-up" className={pad} style={{ ...pc, gridArea: "up" }}><ChevronUp className="h-6 w-6 text-emerald-200" /></button>
          <button {...hold("left")} data-testid="btn-left" className={pad} style={{ ...pc, gridArea: "left" }}><ChevronLeft className="h-6 w-6 text-emerald-200" /></button>
          <button {...hold("down")} data-testid="btn-down" className={pad} style={{ ...pc, gridArea: "down" }}><ChevronDown className="h-6 w-6 text-emerald-200" /></button>
          <button {...hold("right")} data-testid="btn-right" className={pad} style={{ ...pc, gridArea: "right" }}><ChevronRight className="h-6 w-6 text-emerald-200" /></button>
        </div>
        <button {...hold("a")} data-testid="btn-treat" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,210,74,.28), rgba(20,14,4,.9))", boxShadow: "0 0 26px rgba(255,210,74,.4)", touchAction: "none" }}><Bone className="h-6 w-6" style={{ color: "#ffe0a0" }} /> TREAT</button>
      </div>
    </div>
  );
}
