...
import { ArrowLeft, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, ArrowDownToLine } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { SundaeStackEngine } from "@/game/sundae-stack-engine";
import type { Btn } from "@/game/retro-engine";

const GAME_ID = "sundae";
const lsGet = (k: string) => { try { return window.localStorage.getItem(k); } catch { return null; } };

export default function SundaeStack() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<SundaeStackEngine | null>(null);
  ...
  const dstyle = { borderColor: "#3bb6ff", background: "rgba(255,255,255,.03)", boxShadow: "0 0 14px rgba(59,182,255,.22) inset", touchAction: "none" as const, height: 52, width: 52 };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0b0712", color: "#fff4ea", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#3bb6ff", textShadow: "0 0 10px rgba(59,182,255,.5)" }}>Sundae Stack</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="sundae-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(59,182,255,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 grid-rows-3 gap-1" style={{ width: 164 }}>
          <div />
          <button {...hold("up")} data-testid="btn-up" className={dcls} style={dstyle}><ChevronUp className="h-5 w-5 text-sky-300" /></button>
          <div />
          <button {...hold("left")} data-testid="btn-left" className={dcls} style={dstyle}><ChevronLeft className="h-5 w-5 text-sky-300" /></button>
          <div className="flex items-center justify-center text-[8px] font-bold text-sky-300/60">ROT</div>
          <button {...hold("right")} data-testid="btn-right" className={dcls} style={dstyle}><ChevronRight className="h-5 w-5 text-sky-300" /></button>
          <div />
          <button {...hold("down")} data-testid="btn-down" className={dcls} style={dstyle}><ChevronDown className="h-5 w-5 text-sky-300" /></button>
          <div />
        </div>
        <button {...hold("a")} data-testid="btn-drop" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#7be0ff", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(123,224,255,.28), rgba(6,16,20,.9))", boxShadow: "0 0 26px rgba(123,224,255,.4)", touchAction: "none" }}>
          <ArrowDownToLine className="h-7 w-7" style={{ color: "#bfe6ff" }} /> DROP
        </button>
      </div>
    </div>
  );
