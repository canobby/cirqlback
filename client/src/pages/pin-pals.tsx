...
export default function PinPals() {
  ...
  const fl = "flex h-24 flex-1 items-center justify-center rounded-2xl border-[2px] text-[13px] font-extrabold active:scale-95";

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0a0a1a", color: "#dfe8ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffd24a", textShadow: "0 0 10px rgba(255,210,74,.5)" }}>Pin Pals</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="pinpals-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,210,74,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-stretch gap-3 px-5 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <button {...hold("left")} data-testid="btn-flip-l" className={fl} style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 40%, rgba(255,210,74,.24), rgba(20,14,4,.9))", touchAction: "none" }}>◀ FLIP</button>
        <button {...hold("right")} data-testid="btn-flip-r" className={fl} style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 40%, rgba(255,210,74,.24), rgba(20,14,4,.9))", touchAction: "none" }}>FLIP ▶</button>
      </div>
    </div>
  );
}
