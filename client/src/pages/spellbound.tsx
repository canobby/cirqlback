...
import { SpellboundEngine } from "@/game/spellbound-engine";
import { recordRun, recordPoints } from "@/game/rewards";

const GAME_ID = "spellbound";
...
export default function Spellbound() {
  ...
  const engineRef = useRef<SpellboundEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#160c18", color: "#ffe6f5", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[440px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff8ab5", textShadow: "0 0 10px rgba(255,138,181,.5)" }}>Spellbound</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="spellbound-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,138,181,.18)", borderRadius: 6, touchAction: "none", maxHeight: "72vh" }} />
        {over && daily && (
      ...
      <div className="w-full max-w-[440px] px-6 pb-[calc(20px+env(safe-area-inset-bottom))] pt-2 text-center text-[12px] font-bold uppercase tracking-[0.1em]" style={{ color: "#ff8ab5" }}>
        ✦ Tap the keys — or type on your keyboard ✦
      </div>
    </div>
  );
}
