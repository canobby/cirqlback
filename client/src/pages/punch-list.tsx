...
import { PunchListEngine } from "@/game/punch-list-engine";
...
export default function PunchList() {
  ...
  const engineRef = useRef<PunchListEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#120a12", color: "#e6ddf5", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff8ab5", textShadow: "0 0 10px rgba(255,138,181,.5)" }}>Punch List</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="punch-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,138,181,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-3 px-5 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <button {...hold("left")} data-testid="btn-dodge-l" className="flex h-20 w-20 flex-col items-center justify-center rounded-2xl border-[2px] text-[10px] font-extrabold active:scale-95" style={{ borderColor: "#7be0ff", color: "#bfeeff", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronLeft className="h-7 w-7" /> DODGE</button>
        <button {...hold("a")} data-testid="btn-jab" className="flex h-24 w-28 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[14px] font-extrabold active:scale-90" style={{ borderColor: "#ff5d7d", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,93,125,.3), rgba(20,6,12,.9))", boxShadow: "0 0 26px rgba(255,93,125,.4)", touchAction: "none" }}>JAB</button>
        <button {...hold("right")} data-testid="btn-dodge-r" className="flex h-20 w-20 flex-col items-center justify-center rounded-2xl border-[2px] text-[10px] font-extrabold active:scale-95" style={{ borderColor: "#7be0ff", color: "#bfeeff", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronRight className="h-7 w-7" /> DODGE</button>
      </div>
    </div>
  );
}
