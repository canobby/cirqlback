...
import { SnipEngine } from "@/game/snip-engine";
...
export default function Snip() {
  ...
  const engineRef = useRef<SnipEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#120a1a", color: "#e6ddf5", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff8ab5", textShadow: "0 0 10px rgba(255,138,181,.5)" }}>Snip</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="snip-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,138,181,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-center px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <button {...hold("a")} data-testid="btn-snip" className="flex h-24 w-52 items-center justify-center gap-2 rounded-full border-[2px] text-[16px] font-extrabold active:scale-95" style={{ borderColor: "#ff8ab5", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,138,181,.28), rgba(20,6,14,.9))", boxShadow: "0 0 26px rgba(255,138,181,.4)", touchAction: "none" }}>
          <Scissors className="h-7 w-7" style={{ color: "#ffd0e2" }} /> SNIP
        </button>
      </div>
    </div>
  );
