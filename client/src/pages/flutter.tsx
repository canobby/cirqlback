...
export default function Flutter() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0c1420", color: "#eaf6ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[520px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffd24a", textShadow: "0 0 10px rgba(255,210,74,.5)" }}>Flutter</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="flutter-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(59,182,255,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      {/* one button: FLAP (tapping the play area also flaps) */}
      <div className="flex w-full max-w-[520px] items-center justify-center px-6 pb-[calc(20px+env(safe-area-inset-bottom))] pt-2">
        <button {...flap("a")} data-testid="btn-flap" className="flex h-20 w-full max-w-[360px] items-center justify-center rounded-2xl border-[2px] text-[18px] font-extrabold uppercase tracking-wide active:scale-95" style={{ borderColor: "#ffd24a", color: "#0a0714", background: "linear-gradient(180deg,#ffe27a,#ffb020)", boxShadow: "0 0 26px rgba(255,210,74,.4)", touchAction: "none" }}>Flap</button>
      </div>
    </div>
  );
}
