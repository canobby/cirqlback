...
import { ArrowLeft, ChevronLeft, ChevronRight, ChevronDown, RotateCw, ChevronsDown } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { PillPopEngine } from "@/game/pill-pop-engine";
...
export default function PillPop() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<PillPopEngine | null>(null);
  ...
  const sq = "flex h-14 w-14 items-center justify-center rounded-xl border-[1.5px] active:scale-90";

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0a0e18", color: "#e6e9ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#7be0c2", textShadow: "0 0 10px rgba(123,224,194,.5)" }}>Pill Pop</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="pill-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(123,224,194,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      {/* controls: d-pad (left/right/soft-drop) · rotate + hard-drop */}
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="flex gap-2.5">
          <button {...hold("left")} data-testid="btn-left" className={sq} style={{ borderColor: "#3bb6ff", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronLeft className="h-7 w-7 text-cyan-200" /></button>
          <button {...hold("down")} data-testid="btn-down" className={sq} style={{ borderColor: "#3bb6ff", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronDown className="h-7 w-7 text-cyan-200" /></button>
          <button {...hold("right")} data-testid="btn-right" className={sq} style={{ borderColor: "#3bb6ff", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronRight className="h-7 w-7 text-cyan-200" /></button>
        </div>
        <div className="flex items-center gap-3">
          <button {...hold("b")} data-testid="btn-drop" className="flex h-14 w-14 flex-col items-center justify-center rounded-full border-[1.5px] text-[9px] font-extrabold active:scale-90" style={{ borderColor: "#ff8ab5", color: "#ff8ab5", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronsDown className="h-5 w-5" /> DROP</button>
          <button {...hold("a")} data-testid="btn-rotate" className="flex h-20 w-20 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#7be0c2", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(123,224,194,.28), rgba(6,20,16,.9))", boxShadow: "0 0 26px rgba(123,224,194,.4)", touchAction: "none" }}><RotateCw className="h-6 w-6" /> SPIN</button>
        </div>
      </div>
    </div>
  );
