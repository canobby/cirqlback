...
import { ArrowLeft, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Citrus } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { TacoStackEngine } from "@/game/taco-stack-engine";
...
export default function TacoStack() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<TacoStackEngine | null>(null);
  ...
  const dstyle = { borderColor: "#33e650", background: "rgba(255,255,255,.03)", boxShadow: "0 0 14px rgba(51,230,80,.2) inset", touchAction: "none" as const, height: 52, width: 52 };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0b0712", color: "#fff4ea", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#33e650", textShadow: "0 0 10px rgba(51,230,80,.5)" }}>Taco Stack</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="taco-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(51,230,80,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 grid-rows-3 gap-1" style={{ width: 164 }}>
          <div />
          <button {...hold("up")} data-testid="btn-up" className={dcls} style={dstyle}><ChevronUp className="h-5 w-5" style={{ color: "#9bffb0" }} /></button>
          <div />
          <button {...hold("left")} data-testid="btn-left" className={dcls} style={dstyle}><ChevronLeft className="h-5 w-5" style={{ color: "#9bffb0" }} /></button>
          <div />
          <button {...hold("right")} data-testid="btn-right" className={dcls} style={dstyle}><ChevronRight className="h-5 w-5" style={{ color: "#9bffb0" }} /></button>
          <div />
          <button {...hold("down")} data-testid="btn-down" className={dcls} style={dstyle}><ChevronDown className="h-5 w-5" style={{ color: "#9bffb0" }} /></button>
          <div />
        </div>
        <button {...hold("a")} data-testid="btn-lime" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#a4e635", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(164,230,53,.28), rgba(10,16,4,.9))", boxShadow: "0 0 26px rgba(164,230,53,.4)", touchAction: "none" }}>
          <Citrus className="h-7 w-7" style={{ color: "#d9f99d" }} /> LIME
        </button>
      </div>
    </div>
  );
