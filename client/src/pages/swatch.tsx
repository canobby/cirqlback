...
import { ArrowLeft, ChevronLeft, ChevronRight, ChevronDown, Repeat, ChevronsDown } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { SwatchEngine } from "@/game/swatch-engine";
import type { Btn } from "@/game/retro-engine";
import { recordRun, recordPoints } from "@/game/rewards";

const GAME_ID = "swatch";
const lsGet = (k: string) => { try { return window.localStorage.getItem(k); } catch { return null; } };

export default function Swatch() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<SwatchEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0c0e14", color: "#e6e9ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ffd24a", textShadow: "0 0 10px rgba(255,210,74,.5)" }}>Swatch</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="swatch-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,210,74,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="flex gap-2.5">
          <button {...hold("left")} data-testid="btn-left" className={sq} style={{ borderColor: "#ffd24a", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronLeft className="h-7 w-7 text-amber-200" /></button>
          <button {...hold("down")} data-testid="btn-down" className={sq} style={{ borderColor: "#ffd24a", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronDown className="h-7 w-7 text-amber-200" /></button>
          <button {...hold("right")} data-testid="btn-right" className={sq} style={{ borderColor: "#ffd24a", background: "rgba(255,255,255,.03)", touchAction: "none" }}><ChevronRight className="h-7 w-7 text-amber-200" /></button>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button {...hold("a")} data-testid="btn-cycle" className="flex h-20 w-20 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#ffd24a", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,210,74,.28), rgba(20,14,4,.9))", boxShadow: "0 0 26px rgba(255,210,74,.4)", touchAction: "none" }}><Repeat className="h-6 w-6" /> CYCLE</button>
        </div>
      </div>
    </div>
  );
