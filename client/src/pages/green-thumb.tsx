...
import { ArrowLeft, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Crosshair } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { GreenThumbEngine } from "@/game/green-thumb-engine";
import type { Btn } from "@/game/retro-engine";
import { recordRun, recordPoints } from "@/game/rewards";

const GAME_ID = "greenthumb";
...
export default function GreenThumb() {
  ...
  const engineRef = useRef<GreenThumbEngine | null>(null);
  ...
  const pad = "flex h-13 w-13 items-center justify-center rounded-xl border-[1.5px] active:scale-90";
  const pc = { borderColor: "#33e650", background: "rgba(255,255,255,.03)", touchAction: "none" as const, height: 52, width: 52 };

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0a140a", color: "#dff2ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#33e650", textShadow: "0 0 10px rgba(51,230,80,.5)" }}>Green Thumb</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="greenthumb-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(51,230,80,.16)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        ...
        <button {...hold("a")} data-testid="btn-fire" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[13px] font-extrabold active:scale-90" style={{ borderColor: "#33e650", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(51,230,80,.28), rgba(6,20,10,.9))", boxShadow: "0 0 26px rgba(51,230,80,.4)", touchAction: "none" }}><Crosshair className="h-7 w-7" style={{ color: "#c8f7d4" }} /> FIRE</button>
      </div>
    </div>
  );
}
