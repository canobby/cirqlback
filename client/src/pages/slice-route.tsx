...
import { ArrowLeft, ChevronLeft, ChevronRight, Pizza } from "lucide-react";
...
export default function SliceRoute() {
  ...
  const engineRef = useRef<SliceRouteEngine | null>(null);
  ...
  const steerBtn = "flex h-16 w-16 items-center justify-center rounded-2xl border-[1.5px] active:scale-90";
  const steerStyle = { borderColor: "#ff5d7d", background: "rgba(255,255,255,.03)", boxShadow: "0 0 18px rgba(255,93,125,.25) inset", touchAction: "none" as const };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0b0712", color: "#fff4ea", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#ff5d7d", textShadow: "0 0 10px rgba(255,93,125,.5)" }}>Slice Route</div>
      </div>

      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="slice-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(255,93,125,.18)", borderRadius: 6 }} />
        {over && daily && (
      ...
      <div className="flex w-full max-w-[560px] items-center justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="flex gap-2">
          <button {...hold("left")} data-testid="btn-left" className={steerBtn} style={steerStyle}><ChevronLeft className="h-7 w-7 text-rose-300" /></button>
          <button {...hold("right")} data-testid="btn-right" className={steerBtn} style={steerStyle}><ChevronRight className="h-7 w-7 text-rose-300" /></button>
        </div>
        <button {...hold("a")} data-testid="btn-throw" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#ffb020", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(255,176,32,.28), rgba(20,12,6,.9))", boxShadow: "0 0 26px rgba(255,176,32,.4)", touchAction: "none" }}>
          <Pizza className="h-7 w-7" style={{ color: "#ffce9a" }} /> THROW
        </button>
      </div>
    </div>
  );
