...
import { ArrowLeft, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Droplets } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { SparkleEngine } from "@/game/sparkle-engine";
import type { Btn } from "@/game/retro-engine";
import { recordRun, recordPoints } from "@/game/rewards";

const GAME_ID = "sparkle";
const lsGet = (k: string) => { try { return window.localStorage.getItem(k); } catch { return null; } };

export default function Sparkle() {
  const { user } = useAuth();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<SparkleEngine | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center" style={{ background: "#0c1620", color: "#eaf6ff", touchAction: "none", userSelect: "none" }}>
      <div className="flex w-full max-w-[560px] items-center gap-3 px-4 pb-1 pt-3">
        ...
        <div className="ml-1 text-sm font-extrabold uppercase tracking-wide" style={{ color: "#7be0ff", textShadow: "0 0 10px rgba(123,224,255,.5)" }}>Sparkle</div>
      ...
      <div className="relative flex min-h-0 w-full flex-1 items-center justify-center p-2">
        <canvas ref={canvasRef} data-testid="sparkle-canvas" className="block" style={{ imageRendering: "pixelated", boxShadow: "0 0 60px rgba(59,182,255,.18)", borderRadius: 6, touchAction: "none" }} />
        {over && daily && (
      ...
      {/* touch the car to aim + spray · or d-pad the reticle + hold SPRAY */}
      <div className="flex w-full max-w-[560px] items-end justify-between gap-4 px-6 pb-[calc(18px+env(safe-area-inset-bottom))] pt-2">
        <div className="grid grid-cols-3 grid-rows-2 gap-1.5" style={{ width: 148 }}>
          ...
          <button {...hold("left")} data-testid="btn-left" className={padBtn} style={padStyle}><ChevronLeft className="h-6 w-6 text-cyan-200" /></button><button {...hold("down")} data-testid="btn-down" className={padBtn} style={padStyle}><ChevronDown className="h-6 w-6 text-cyan-200" /></button><button {...hold("right")} data-testid="btn-right" className={padBtn} style={padStyle}><ChevronRight className="h-6 w-6 text-cyan-200" /></button>
        </div>
        <button {...hold("a")} data-testid="btn-spray" className="flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-full border-[2px] text-[12px] font-extrabold active:scale-90" style={{ borderColor: "#3bb6ff", color: "#fff", background: "radial-gradient(circle at 50% 38%, rgba(59,182,255,.3), rgba(12,22,32,.9))", boxShadow: "0 0 26px rgba(59,182,255,.4)", touchAction: "none" }}><Droplets className="h-6 w-6" /> SPRAY</button>
      </div>
    </div>
  );
}
