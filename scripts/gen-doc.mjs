// Generic Markdown -> branded print-ready PDF renderer.
//   node scripts/gen-doc.mjs <src.md> <OutName>
// Same self-contained md->HTML + headless-Chromium print as gen-legal/gen-beta,
// with GitHub-style task-list checkboxes (- [ ] / - [x]) and page-breaks before
// each top-level (#) section. Output lands in docs/cirql/pdf/<OutName>.pdf.
...
const OUT_NAME = process.argv[3] || "document";
const OUT_DIR = "docs/cirql/pdf";
if (!SRC || !existsSync(SRC)) { console.error("usage: node scripts/gen-doc.mjs <src.md> <OutName>"); process.exit(1); }
...
function md2html(md) {
  ...
  while (i < lines.length) {
    ...
    if (t.startsWith(">")) { const buf = []; while (i < lines.length && lines[i].trim().startsWith(">")) { buf.push(lines[i].trim().replace(/^>\s?/, "")); i++; } html += `<blockquote>${buf.map((b) => `<p>${inline(b)}</p>`).join("")}</blockquote>`; continue; }
    if (t.startsWith("|") && i + 1 < lines.length && /^\|?[\s:|-]+\|?$/.test(lines[i + 1].trim()) && lines[i + 1].includes("-")) {
      ...
      html += `<table><thead><tr>${header.map((c) => `<th>${inline(c)}</th>`).join("")}</tr></thead><tbody>${rows.map((r) => `<tr>${r.map((c) => `<td>${inline(c)}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
      continue;
    }
    if (/^\s*([-*]|\d+[.)])\s+/.test(line)) {
      const ordered = /^\s*\d+[.)]\s+/.test(line); const items = [];
      while (i < lines.length && /^\s*([-*]|\d+[.)])\s+/.test(lines[i])) { items.push(lines[i].replace(/^\s*([-*]|\d+[.)])\s+/, "")); i++; }
      const tag = ordered ? "ol" : "ul"; const hasTask = items.some((it) => /^\[[ xX]\]\s+/.test(it));
...
function template(body) {
  ...
  * { box-sizing:border-box; }
  ...
  th { background:#f6f4fc; color:#5b21b6; } strong { color:#0f1220; }
  </style></head><body>${body}</body></html>`;
}
function findBrowser() {
  return [process.env.CHROME_PATH, "C:/Program Files/Google/Chrome/Application/chrome.exe", "C:/Program Files (x86)/Google/Chrome/Application/chrome.exe", "C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe", "C:/Program Files/Microsoft/Edge/Application/msedge.exe", "/usr/bin/google-chrome", "/usr/bin/chromium"].filter(Boolean).find((p) => existsSync(p));
}
const browser = findBrowser();
if (!browser) { console.error("No Chrome/Edge found. Set CHROME_PATH."); process.exit(1); }
if (!existsSync(OUT_DIR)) mkdirSync(OUT_DIR, { recursive: true });
const tmp = tmpdir();
const htmlPath = join(tmp, `cirq-doc-${Date.now()}.html`);
...
execFileSync(browser, ["--headless", "--disable-gpu", "--no-first-run", "--no-default-browser-check", `--user-data-dir=${profile}`, "--no-pdf-header-footer", `--print-to-pdf=${pdf}`, "file:///" + htmlPath.replace(/\\/g, "/")], { stdio: "ignore" });
