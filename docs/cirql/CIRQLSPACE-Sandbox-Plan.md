...
**Locked decisions (owner, 2026-07-07):** (1) **sequence = I → J → K → F → G** (movement first — cheap + elevates everything; F after K since it consumes K1's NPC framework; G last to score it all); (2) **new "Renown"** currency (personal, earned-not-spent, gates advanced campaigns/stores/rare décor) **+ keep the shared World Energy meter** — sparqs stay purely for spending; (3) **tap-ready but wire later** — build campaigns + Renown + World Energy so real taps *can* feed them, but advance on in-game play for now; flip the real-tap source on in a dedicated economy pass. Pillar confirmed: **fully cozy, no combat.**
