...
- **M11 · Depth & Delight — DONE (7/7):** **CHR-265 Interactive voyages** (`be24ce3`) — `portal` props → cave/treetop/cloud **sub-maps**. **CHR-260 Emotes** (`4749427`) — 12-emote wheel → glyph over local+remote avatars via `/ws/cirql`. **CHR-264 Cutscenes** (`ff50176`) — reusable skippable letterboxed cutscene player (`cirql-cutscenes.ts`), wired to arrival/onboarding/campaign/world-energy. **CHR-262 Sailing voyage** (`41dd953`) — surface↔surface sailing is now a steerable crossing (gather drifting light, island grows through fog) → land + arrival cutscene; portals/sub-maps still swap instantly. **CHR-263 Jump/hop** (`5df8e39`) — fake-Z hop (raised sprite, grounded shadow) + HOP button + C/H key. **CHR-259 Hearth décor + visiting** (`a9badec`) — spark-bought décor catalog (`cirql-decor.ts`), place/remove on your Hearth (persisted), + presence-broadcast **visiting** of online friends' Hearths. **CHR-261 Tapped businesses → landmarks** (`b5b0068`) — your real tapped businesses become signpost landmarks ringing your Hearth (`GET /api/cirql/landmarks`; category→glyph); the loyalty↔world moat made visible.
...
## M11 · Depth & Delight (wishlist — owner-approved) — DONE (7/7)
...
- [x] **Interactive voyages: tunnels/underground, treetops & clouds** *(CHR-265)*
- [x] **Emotes + chat-free expression** *(CHR-260)* — 12-emote wheel → glyph over local/remote avatars + `/ws/cirql` relay
- [x] **More mini-journeys & cutscenes** *(CHR-264)* — reusable skippable cutscene player; arrival / onboarding / campaign / world-energy
- [x] **Sailing as a mini-journey** *(CHR-262)* — steerable sea crossing, gather light, island grows through fog → land
- [x] **Jump / hop movement** *(CHR-263)* — fake-Z hop (raised sprite, grounded shadow) + HOP button + C/H key
- [x] **Hearth décor + visiting friends' Hearths** *(CHR-259)* — spark-bought décor place/remove/persist + presence-broadcast visiting
- [x] **Tapped businesses become in-world landmarks** *(CHR-261)* — real taps → signpost landmarks ringing your Hearth (loyalty↔world)
