...
- [x] **K5** — **Flagship journeys DONE** (`e965cc4`): `cirql-journeys.ts` — world-spanning personal campaigns tracking a cumulative metric with Renown milestones: **The Widening Sea** (rings reached), **Cartographer's Circles** (circles graduated), **Founding a Homestead** (land tier), **The Keeper's Ledger** (quests done). `checkJourneys()` grants Renown once/milestone (claims persisted; pre-journeys saves seed silently); wired to quest-complete/sail/land-expand; Journeys section in the (now scrollable) Quests panel with progress bars. Verified milestone payout. *(Co-op multi-day campaigns still ride the M9 party board.)*
- [~] **K6** — Seasonal festivals. **REAL-WORLD HOLIDAYS DONE** (`f31c931`, owner-requested): on the actual calendar day, the daily is re-themed + sparq-boosted — New Year's(3×)/Valentine's/Easter/Memorial/July 4th/Labor/Halloween/Thanksgiving/Christmas Eve/Christmas(3×), each w/ emoji+accent+blurb. `holidayFor(date)` (fixed + computed floating: Easter algo, nth/last-weekday); `activeEvent()` returns holiday>weekend-festival; daily card + banner theme automatically + show the emoji. *(Deferred: the 4 lore festivals [Lantern/Bloom/Starfall/Hollow Night] + exclusive cosmetics.)*
