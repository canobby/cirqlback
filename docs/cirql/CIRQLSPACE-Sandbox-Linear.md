...
- [~] **K3** — Quest taxonomy + **DIFFICULTY/REWARD SCALING DONE** (`cd4ff85`): `generateRingQuest` now ramps by ring tier — sparqs `6+tier*3` (cap 60, was 16), kindle counts grow, and escalation objectives stack (tier≥2 → visit the J4 landmark; tier≥4 → chart the onward shore) so far quests are multi-step. `QuestDef.tier`; quest log shows Tier badge + steps + ✦reward. Verified ring 2=T1/1-step/✦9 → ring 18=T17/3-step/✦57. *(Deferred: the richer taxonomy — gather/deliver/talk-chain/build/photo-via-diorama/social.)*
- [ ] **K4** — Daily board + **Renown** ladder (character-bound dailies → sparqs+streak; Renown gates advanced campaigns/stores/rare décor; Renown also fed by real taps — the moat). *(Renown reward on quests + gating = the natural next K step, pairs with the K3 scaling.)*
